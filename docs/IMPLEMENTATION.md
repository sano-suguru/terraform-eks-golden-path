...
このドキュメントでは、設計時に考えたことや実装のポイントをまとめています。
...
依存関係は「外側から内側へ」の一方向にしています。アプリケーションは Kubernetes に依存せず、Kubernetes マニフェストは Terraform に依存しません。

## Golden Path（標準ルート）

### ロギング

JSON 構造化ログに統一しました。後から集約・分析しやすくするためです。
...
`/healthz`, `/readyz`, `/metrics` はログを出さないようにしています。Kubernetes の probe が大量に来るので、ノイズになるためです。

### メトリクス

Prometheus 形式で統一しました。
...
**Liveness と Readiness を明確に分離しました。**
...
**Helm チャートを kind/EKS で共通化し、環境差分は values ファイルで吸収するようにしました。**

「ローカルでは動いたのに本番で動かない」を避けるためです。
...
このリポジトリでは、以下のような仕組みを入れています：

| やりたかったこと | やったこと |
|-----------------|-----------|
| ログ形式を統一したい | JSON 構造化ログに統一 |
| メトリクスを統一したい | Prometheus 形式で統一 |
| ヘルスチェックを明確にしたい | Liveness/Readiness を分離 |
| 環境差分を吸収したい | Helm チャートを共通化 |
| セキュリティ設定を属人化させない | CI で自動チェック（Trivy, OPA） |
| 品質を人の善意に頼らない | Guardrails で CI 必須化 |
